 ...
 * 泛型E用来代表数据
 */
abstract class WDPresenter<T, E> constructor(private val dataCall: DataCall<E>) {
    ...
    protected abstract fun getModel(args: List<Any>): Observable<Result<E>>
...
//                .onErrorReturn {  }
                .subscribe({ result ->
                    ...
                    if (result.status == "0000") {
                        dataCall.success(result.result, args.asList())
                    } else {
                        dataCall.fail(ApiException(result.status, result.message), args.asList())
                    }
                },
                        ({ e ->
                            e.printStackTrace()
                            dataCall.fail(CustomException.handleException(e), args.asList())
                        }))
    }
